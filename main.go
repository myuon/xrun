...
func main() {
	...
	if execTemplate != "" && inputFile != "" {
		fmt.Fprintf(os.Stderr, "Error: Cannot specify both -e and -i flags\n")
		os.Exit(1)
	}

	// Read command template from input file if -i flag is used
	if inputFile != "" {
		...
		execTemplate = strings.TrimSpace(string(templateBytes))
	}
